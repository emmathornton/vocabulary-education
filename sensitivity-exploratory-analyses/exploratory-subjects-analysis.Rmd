...
#combine gcse and n5 so can select analytical sample
vocabulary_education$english_n5 = as.numeric(vocabulary_education$english_n5)
vocabulary_education$maths_n5 = as.numeric(vocabulary_education$maths_n5)
vocabulary_education$science_n5 = as.numeric(vocabulary_education$science_n5)
vocabulary_education = 
  vocabulary_education %>% mutate(english = case_when(
    !is.na(english_gcse) ~ english_gcse,
    is.na(english_gcse) ~ english_n5, 
    !is.na(english_n5) ~ english_n5,
    is.na(english_n5) ~ english_gcse)) %>% 
  mutate(maths = 
           case_when(!is.na(maths_gcse)~maths_gcse,
                     is.na(maths_gcse) ~ maths_n5, 
                     !is.na(maths_n5) ~maths_n5,
                     is.na(maths_n5) ~ maths_gcse)) %>% 
  mutate(science = 
           case_when(!is.na(science_gcse)~science_gcse,
                     is.na(science_gcse) ~ science_n5, 
                     !is.na(science_n5) ~science_n5,
                     is.na(science_n5) ~ science_gcse)) %>% 
  select(-english_gcse, -english_n5, -maths_gcse, -maths_n5, -science_gcse, -science_n5)


exploratory = vocabulary_education %>% filter(!is.na(age5_vocab) | !is.na(english) |
                                                             !is.na(maths) | !is.na(science))
...
continuous_unadjustedEnglish = with(imputed_mcs2, lm(standardised_english ~ standardised_age5_vocab, weights = weight))
...
maths
```{r}
continuous_unadjustedMaths = with(imputed_mcs2, lm(standardised_maths ~ standardised_age5_vocab, weights = weight))
...
science 
```{r}
continuous_unadjustedScience = with(imputed_mcs2, lm(standardised_science ~ standardised_age5_vocab, weights = weight))
...
continuous_model1ResultsEnglish = continuous_model1ResultsEnglish %>% add_row(term = "standardised_caregiver_vocab") %>% 
...
continuous_model2ResultsEnglish = continuous_model2ResultsEnglish %>% add_row(term = "standardised_age5_vocab") %>% 
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsMaths = continuous_model1ResultsMaths %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
  add_row(term = "R2", model1_maths =paste0(cont_model1R2Maths$est,"[", cont_model1R2Maths$`lo 95`, ";", cont_model1R2Maths$`hi 95`,"]"), .after = 39)
continuous_model2ResultsMaths = continuous_model2ResultsMaths %>% add_row(term = "standardised_age5_vocab") %>% 
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsScience = continuous_model1ResultsScience %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
...
continuous_model2ResultsScience = continuous_model2ResultsScience %>% add_row(term = "standardised_age5_vocab") %>% 
...
results_exploratory <- regression_resultsTable %>% 
 ...
 add_header_row(values = c(" ", "English Outcome (B[95% CIs]", "Maths Outcome (B[95% CIs]", "Science Outcome (B[95% CIs]"), colwidths = c(1, 3, 3,3)) %>% 
  ...
  add_header_lines(values = c("Table S9: Predicting Educational Attainment in English, Maths and Science (N=15,475)"))
