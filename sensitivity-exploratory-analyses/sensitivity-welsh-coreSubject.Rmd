...
welsh_sensitivity = vocabulary_education %>% filter(!is.na(age5_vocab) | (!is.na(benchmark_binary_welsh)))
welshSensitivity = long_format_mcs[long_format_mcs$mcsid %in% welsh_sensitivity$mcsid,] 
imputed_mcs2 = as.mids(welshSensitivity)
...
binary_unadjusted = with(imputed_mcs2, glm(benchmark_binary_welsh ~ standardised_age5_vocab,
...
continuous_unadjusted = with(imputed_mcs2, lm(standardised_core_subjectsWelsh ~ standardised_age5_vocab, weights = weight))
...
results_Rq1Rq2 <- regression_resultsTable %>% 
 ...
 add_header_row(values = c(" ", "Binary Outcome (OR[95% CIs]", "Continuous Outcome (B[95% CIs]"), colwidths = c(1, 3, 3)) %>% 
  ...
  add_header_lines(values = c("Table S3: Predicting Educational Attainment (≥grade 4 on core subjects and average grade, including Welsh as a core subject) (N=15,576)"))
