...
results_england <- regression_resultsTableEngland %>% 
 ...
 add_header_row(values = c(" ", "Binary Outcome (OR[95% CIs]", "Continuous Outcome (B[95% CIs]"), colwidths = c(1, 3, 3)) %>% 
  ...
  add_header_lines(values = c("Table S4: Predicting Educational Attainment in England (≥grade 4 on core subjects and average grade) (N=10,076)"))
...
results_wales <- regression_resultsTableWales %>% 
  ...
  add_header_lines(values = c("Table S5: Predicting Educational Attainment in Wales (≥grade 4 on core subjects and average grade) (N=2,144)"))
...
results_scotland <- regression_resultsTableScotland %>% 
  ...
  bold(j = 1, i = c(1,39, 41), bold = TRUE) %>% 
  italic(j = 1, i = c(1,39, 41), italic = TRUE) %>% 
  add_header_lines(values = c("Table S6: Predicting Educational Attainment in Scotland (≥grade 4 on core subjects and average grade) (N=1,821)"))
...
results_northern_ireland <- regression_resultsTableNi %>% 
  ...
  bold(j = 1, i = c(1,39, 41), bold = TRUE) %>% 
  italic(j = 1, i = c(1,39, 41), italic = TRUE)  %>% 
  add_header_lines(values = c("Table S7: Predicting Educational Attainment in Northern Ireland (≥grade 4 on core subjects and average grade) (N=1,535)"))
...
nocompositeModerator_england<- function(df) {
  fit1 <- glm(success ~ sex + ethnicity + EAL + 
                + caregiver_vocabStandardised +  ses_latent + age5_standardised , 
              family = binomial, weights = weight, data=df)
  return(fit1)
}
...
nocompositeModerator_wales<- function(df) {
  fit1 <- glm(success ~ sex + ethnicity_binary + EAL + 
                + caregiver_vocabStandardised +  ses_latent + age5_standardised , 
              family = binomial, weights = weight, data=df)
  return(fit1)
}
