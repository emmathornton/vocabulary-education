...
binary_unadjustedEngland = with(england_imputed, glm(benchmark_binary ~ standardised_age5_vocab,
...
binary_model1England = with(england_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary + 
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                       family = binomial, weights = weight))

binary_model2England= with(england_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                         imd + standardised_caregiver_vocab, 
                                       family = binomial, weights = weight))

binary_model3England = with(england_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
...
continuous_unadjustedEngland = with(england_imputed, lm(standardised_core_subjects ~ standardised_age5_vocab, weights = weight))
...
continuous_model1England = with(england_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                       weights = weight))

continuous_model2England = with(england_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                         imd + standardised_caregiver_vocab, 
                                        weights = weight))

continuous_model3England = with(england_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                         highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                         imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                        weights = weight))
...
binary_model1ResultsEngland = binary_model1ResultsEngland %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab")
binary_model2ResultsEngland = binary_model2ResultsEngland %>% add_row(term = "standardised_age5_vocab")
...
binary_outcomeResultsEngland = binary_outcomeResultsEngland %>% 
  add_row(term = "Sociodemographic confounders", .after = 1) %>% 
  add_row(term = "Caregiver vocabulary", .before = 31) %>% 
  add_row(term = "Cohort member vocabulary", .before  = 33) %>% 
  add_row(term = "R2", .after = 34)
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsEngland = continuous_model1ResultsEngland %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
  add_row(term = "R2", model1_b =paste0(cont_model1R2England$est,"[", cont_model1R2England$`lo 95`, ";", cont_model1R2England$`hi 95`,"]"), .after = 39)
continuous_model2ResultsEngland = continuous_model2ResultsEngland %>% add_row(term = "standardised_age5_vocab") %>% 
...
regression_resultsTableEngland = regression_resultsTableEngland %>% select(!5) %>%  
  ...
  add_row(term = "highest_nvq1", .after = 7) %>% 
  add_row(term = "oecd_income1", .before = 14) %>% 
  add_row(term = "occupational_status2", .before = 19) %>% 
  add_row(term = "wealth_quintiles1", .before = 23) %>% 
  add_row(term = "imd1", .before = 28)
...
results_england <- regression_resultsTableEngland %>% 
 ...
 add_header_row(values = c(" ", "Binary Outcome (OR[95% CIs]", "Continuous Outcome (B[95% CIs]"), colwidths = c(1, 3, 3)) %>% 
  ...
  bold(j = 1, i = c(1,38, 40), bold = TRUE) %>% 
  italic(j = 1, i = c(1,38, 40), italic = TRUE) %>% 
  add_header_lines(values = c("Table S5: Predicting Educational Attainment in England (≥grade 4 on core subjects and average grade) (N=10,076)"))
  
  

 print(results_england, preview = "docx") 
```
...
#1.binary outcome of core subjects 
binary_unadjustedWales = with(wales_imputed, glm(benchmark_binary ~ standardised_age5_vocab,
                                                     family = binomial,  weights = weight))

binary_unadjusted_resultsWales = round(summary(pool(binary_unadjustedWales), conf.int = TRUE, exponentiate = TRUE),2)

binary_model1Wales = with(wales_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary + 
                                                   highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                                 family = binomial, weights = weight))

binary_model2Wales= with(wales_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                  imd + standardised_caregiver_vocab, 
                                                family = binomial, weights = weight))

binary_model3Wales = with(wales_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                                   highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                   imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                                 family = binomial, weights = weight))
...
continuous_unadjustedWales = with(wales_imputed, lm(standardised_core_subjects ~ standardised_age5_vocab, weights = weight))

continuous_unadjusted_resultsWales = round(summary(pool(continuous_unadjustedWales), conf.int = TRUE),2)

#adjusted relationship 
continuous_model1Wales = with(wales_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                      highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                                    weights = weight))

continuous_model2Wales = with(wales_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                      highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                      imd + standardised_caregiver_vocab, 
                                                    weights = weight))

continuous_model3Wales = with(wales_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                      highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                      imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                                    weights = weight))
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
binary_model1ResultsWales = binary_model1ResultsWales %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab")
binary_model2ResultsWales = binary_model2ResultsWales %>% add_row(term = "standardised_age5_vocab")
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsWales = continuous_model1ResultsWales %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
  add_row(term = "R2", model1_b =paste0(cont_model1R2Wales$est,"[", cont_model1R2Wales$`lo 95`, ";", cont_model1R2Wales$`hi 95`,"]"), .after = 32)
continuous_model2ResultsWales = continuous_model2ResultsWales %>% add_row(term = "standardised_age5_vocab") %>% 
...
results_wales <- regression_resultsTableWales %>% 
  ...
  bold(j = 1, i = c(1,38, 40), bold = TRUE) %>% 
  italic(j = 1, i = c(1,38, 40), italic = TRUE) %>% 
  add_header_lines(values = c("Table S6: Predicting Educational Attainment in Wales (≥grade 4 on core subjects and average grade) (N=2,144)"))
...
#1.binary outcome of core subjects 
binary_unadjustedScotland = with(scotland_imputed, glm(benchmark_binary ~ standardised_age5_vocab,
                                                 family = binomial,  weights = weight))

binary_unadjusted_resultsScotland = round(summary(pool(binary_unadjustedScotland), conf.int = TRUE, exponentiate = TRUE),2)

binary_model1Scotland = with(scotland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary + 
                                               highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                             family = binomial, weights = weight))

binary_model2Scotland= with(scotland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                              highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                              imd + standardised_caregiver_vocab, 
                                            family = binomial, weights = weight))

binary_model3Scotland = with(scotland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                               highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                               imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                             family = binomial, weights = weight))
...
continuous_unadjustedScotland = with(scotland_imputed, lm(standardised_core_subjects ~ standardised_age5_vocab, weights = weight))

continuous_unadjusted_resultsScotland = round(summary(pool(continuous_unadjustedScotland), conf.int = TRUE),2)

#adjusted relationship 
continuous_model1Scotland = with(scotland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                                weights = weight))

continuous_model2Scotland = with(scotland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                  imd + standardised_caregiver_vocab, 
                                                weights = weight))

continuous_model3Scotland = with(scotland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                  imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                                weights = weight))
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
binary_model1ResultsScotland = binary_model1ResultsScotland %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab")
binary_model2ResultsScotland = binary_model2ResultsScotland %>% add_row(term = "standardised_age5_vocab")
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsScotland = continuous_model1ResultsScotland %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
...
continuous_model2ResultsScotland = continuous_model2ResultsScotland %>% add_row(term = "standardised_age5_vocab") %>% 
...
continuous_outcomeResultsScotland = continuous_outcomeResultsScotland %>% add_row(term = "Sociodemographic confounders", .after = 1) %>% 
  add_row(term = "Caregiver vocabulary", .after = 30) %>% 
  add_row(term = "Cohort member vocabulary", .before  = 33)
...
results_scotland <- regression_resultsTableScotland %>% 
  ...
  bold(j = 1, i = c(1,38, 40), bold = TRUE) %>% 
  italic(j = 1, i = c(1,38, 40), italic = TRUE) %>% 
  add_header_lines(values = c("Table S7: Predicting Educational Attainment in Scotland (≥grade 4 on core subjects and average grade) (N=1,821)"))
...
#1.binary outcome of core subjects 
binary_unadjustedNi = with(northern_ireland_imputed, glm(benchmark_binary ~ standardised_age5_vocab,
                                                 family = binomial,  weights = weight))

binary_unadjusted_resultsNi = round(summary(pool(binary_unadjustedNi), conf.int = TRUE, exponentiate = TRUE),2)

binary_model1Ni = with(northern_ireland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary + 
                                               highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                             family = binomial, weights = weight))

binary_model2Ni= with(northern_ireland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                              highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                              imd + standardised_caregiver_vocab, 
                                            family = binomial, weights = weight))

binary_model3Ni = with(northern_ireland_imputed, glm(benchmark_binary ~ sex + ethnicity_binary + EAL_binary +
                                               highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                               imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                             family = binomial, weights = weight))
...
continuous_unadjustedNi = with(northern_ireland_imputed, lm(standardised_core_subjects ~ standardised_age5_vocab, weights = weight))

continuous_unadjusted_resultsNi = round(summary(pool(continuous_unadjustedNi), conf.int = TRUE),2)

#adjusted relationship 
continuous_model1Ni = with(northern_ireland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + imd, 
                                                weights = weight))

continuous_model2Ni = with(northern_ireland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary+ 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                  imd + standardised_caregiver_vocab, 
                                                weights = weight))

continuous_model3Ni = with(northern_ireland_imputed, lm(standardised_core_subjects ~ sex + ethnicity_binary + EAL_binary + 
                                                  highest_nvq + oecd_income + occupational_status + wealth_quintiles + 
                                                  imd + standardised_caregiver_vocab + standardised_age5_vocab, 
                                                weights = weight))
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
binary_model1ResultsNi = binary_model1ResultsNi %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab")
binary_model2ResultsNi = binary_model2ResultsNi %>% add_row(term = "standardised_age5_vocab")
...
#add column to make it the same length for all 3 models - needs to be as long as model 3 
continuous_model1ResultsNi = continuous_model1ResultsNi %>% add_row(term = "standardised_caregiver_vocab") %>% 
  add_row(term = "standardised_age5_vocab") %>% 
  add_row(term = "R2", model1_b =paste0(cont_model1R2Ni$est,"[", cont_model1R2Ni$`lo 95`, ";", cont_model1R2Ni$`hi 95`,"]"), .after = 32)
continuous_model2ResultsNi = continuous_model2ResultsNi %>% add_row(term = "standardised_age5_vocab") %>% 
...
results_northern_ireland <- regression_resultsTableNi %>% 
  ...
  bold(j = 1, i = c(1,38, 40), bold = TRUE) %>% 
  italic(j = 1, i = c(1,38, 40), italic = TRUE)  %>% 
  add_header_lines(values = c("Table S8: Predicting Educational Attainment in Northern Ireland (≥grade 4 on core subjects and average grade) (N=1,535)"))
...
compositeModerator_england <- function(df) {
  fit <- glm(success ~ sex + ethnicity_binary + EAL_binary + 
...
nocompositeModerator_england<- function(df) {
  fit1 <- glm(success ~ sex + ethnicity_binary + EAL_binary + 
                + standardised_caregiver_vocab +  ses_latent + standardised_age5_vocab , 
              family = binomial, weights = weight, data=df)
  return(fit1)
}
...
plot for moderation analysis in each country 
```{r}
#plots####

#england 

compositeQuintiles_englandModel <- function(df) {
  fit <- glm(success ~ sex + ethnicity + EAL + 
...
#put new imputed datasets into lists (i.e. ones with the latent variable in them) and apply regression model to each
compositeQuintiles_england <- lapply(list(imputed_dataEngland1, imputed_dataEngland2, imputed_dataEngland3, imputed_dataEngland4, imputed_dataEngland5,
                                  imputed_dataEngland6, imputed_dataEngland7, imputed_dataEngland8, imputed_dataEngland9, imputed_dataEngland10,
                                  imputed_dataEngland11, imputed_dataEngland12, imputed_dataEngland13, imputed_dataEngland14, imputed_dataEngland15,
                                  imputed_dataEngland16, imputed_dataEngland17, imputed_dataEngland18, imputed_dataEngland19, imputed_dataEngland20,
                                  imputed_dataEngland21, imputed_dataEngland22, imputed_dataEngland23, imputed_dataEngland24, imputed_dataEngland25),
                             compositeQuintiles_englandModel)
#get probabilities
composite_quintiles_england = lapply(compositeQuintiles_england, ggpredict, terms = c("standardised_age5_vocab", "composite_quintiles"))


#wales
compositeQuintiles_walesModel <- function(df) {
  fit <- glm(success ~ sex + ethnicity_binary + EAL + 
               + standardised_caregiver_vocab + composite_quintiles*standardised_age5_vocab, 
             family = binomial, weights = weight, data=df)
  return(fit)
}
...
#put new imputed datasets into lists (i.e. ones with the latent variable in them) and apply regression model to each
compositeQuintiles_wales <- lapply(list(imputed_dataWales1, imputed_dataWales2, imputed_dataWales3, imputed_dataWales4, imputed_dataWales5,
                                          imputed_dataWales6, imputed_dataWales7, imputed_dataWales8, imputed_dataWales9, imputed_dataWales10,
                                          imputed_dataWales11, imputed_dataWales12, imputed_dataWales13, imputed_dataWales14, imputed_dataWales15,
                                          imputed_dataWales16, imputed_dataWales17, imputed_dataWales18, imputed_dataWales19, imputed_dataWales20,
                                          imputed_dataWales21, imputed_dataWales22, imputed_dataWales23, imputed_dataWales24, imputed_dataWales25),
                                     compositeQuintiles_walesModel)
#get probabilities
composite_quintiles_wales = lapply(compositeQuintiles_wales, ggpredict, terms = c("standardised_age5_vocab", "composite_quintiles"))

#scotland
compositeQuintiles_scotlandModel <- function(df) {
  fit <- glm(success ~ sex + ethnicity_binary + EAL + 
               + standardised_caregiver_vocab + composite_quintiles*standardised_age5_vocab, 
             family = binomial, weights = weight, data=df)
  return(fit)
...
#put new imputed datasets into lists (i.e. ones with the latent variable in them) and apply regression model to each
compositeQuintiles_scotland <- lapply(list(imputed_dataScotland1, imputed_dataScotland2, imputed_dataScotland3, imputed_dataScotland4, imputed_dataScotland5,
                                          imputed_dataScotland6, imputed_dataScotland7, imputed_dataScotland8, imputed_dataScotland9, imputed_dataScotland10,
                                          imputed_dataScotland11, imputed_dataScotland12, imputed_dataScotland13, imputed_dataScotland14, imputed_dataScotland15,
                                          imputed_dataScotland16, imputed_dataScotland17, imputed_dataScotland18, imputed_dataScotland19, imputed_dataScotland20,
                                          imputed_dataScotland21, imputed_dataScotland22, imputed_dataScotland23, imputed_dataScotland24, imputed_dataScotland25),
                                     compositeQuintiles_scotlandModel)
#get probabilities
composite_quintiles_scotland = lapply(compositeQuintiles_scotland, ggpredict, terms = c("standardised_age5_vocab", "composite_quintiles"))


#northern ireland
compositeQuintiles_northern_irelandModel <- function(df) {
  fit <- glm(success ~ sex + ethnicity_binary + EAL +
               + standardised_caregiver_vocab + composite_quintiles*standardised_age5_vocab, 
             family = binomial, weights = weight, data=df)
  return(fit)
...
#put new imputed datasets into lists (i.e. ones with the latent variable in them) and apply regression model to each
compositeQuintiles_northern_ireland <- lapply(list(imputed_dataNI1, imputed_dataNI2, imputed_dataNI3, imputed_dataNI4, imputed_dataNI5,
                                  imputed_dataNI6, imputed_dataNI7, imputed_dataNI8, imputed_dataNI9, imputed_dataNI10,
                                  imputed_dataNI11, imputed_dataNI12, imputed_dataNI13, imputed_dataNI14, imputed_dataNI15,
                                  imputed_dataNI16, imputed_dataNI17, imputed_dataNI18, imputed_dataNI19, imputed_dataNI20,
                                  imputed_dataNI21, imputed_dataNI22, imputed_dataNI23, imputed_dataNI24, imputed_dataNI25),
                             compositeQuintiles_northern_irelandModel)

#get predicted probabilities
composite_quintiles_northern_ireland = lapply(compositeQuintiles_northern_ireland, ggpredict, terms = c("standardised_age5_vocab", "composite_quintiles"))

#plot 
 
england_plot = plot(pool_predictions(composite_quintiles_england), show_ci = FALSE,
                               limits = c(0, 1), 
                               breaks = seq(0, 1, by = 0.1)) +
  scale_colour_manual(values=c("#f17300", "#f6a55c", "#9dbcd7", "#6596c1","#054a91"), 
                      name="SEC factor score", 
                      breaks=c("5", "4","3", "2", "1"),
                      labels=c("Highest Quintile", "Quintile 4", "Quintile 3", "Quintile 2", "Lowest Quintile"))+
  scale_fill_manual(values=c("#f17300", "#f6a55c", "#9dbcd7", "#6596c1","#054a91"), 
                    name="SEC factor score",
                    breaks=c("5", "4","3", "2", "1"),
                      labels=c("Highest Quintile", "Quintile 4", "Quintile 3", "Quintile 2", "Lowest Quintile"))+
  labs(x = "Age 5 Vocabulary Score \n (Standardised)", 
       ...
       title = "England") +
  labs(colour = "SEC factor score") +
  theme_classic() +
  scale_x_continuous(breaks=seq(-3, 4, .5)) +
  theme(panel.grid.major.y = element_line(colour = "gainsboro", size = 0.2),
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank(), 
        plot.title = element_text(size = 12, face = "italic", family = "Times")) 

wales_plot = plot(pool_predictions(composite_quintiles_wales), show_ci = FALSE,
                               limits = c(0, 1), 
                               breaks = seq(0, 1, by = 0.1)) +
  scale_colour_manual(values=c("#f17300", "#f6a55c", "#9dbcd7", "#6596c1","#054a91"), 
                      name="SEC factor score", 
...
scotland_plot = plot(pool_predictions(composite_quintiles_scotland), show_ci = FALSE,
                               limits = c(0, 1), 
                               breaks = seq(0, 1, by = 0.1)) +
  scale_colour_manual(values=c("#f17300", "#f6a55c", "#9dbcd7", "#6596c1","#054a91"), 
                       name="SEC factor score", 
...
NI_plot = plot(pool_predictions(composite_quintiles_northern_ireland), show_ci = FALSE,
                               limits = c(0, 1), 
                               breaks = seq(0, 1, by = 0.1)) +
  scale_colour_manual(values=c("#f17300", "#f6a55c", "#9dbcd7", "#6596c1","#054a91"), 
                      name="SEC factor score", 
...
plots = ggarrange(england_plot, wales_plot, scotland_plot, NI_plot, 
                  ncol =2, nrow=2,  align = c( "hv"))

plots = annotate_figure(plots, top = text_grob("Figure S3: Predicted probabilities of achieving benchmark qualifications for vocabulary moderated by SEC in each country", 
                                                      color = "black", face = "italic", size = 20, family = "Times"))

plots = plots + labs(caption ="*Predicted probabilities when categorical potential confounders (sex, ethnicity and EAL status set to reference levels and mean caregiver vocabulary score.") +
  theme(plot.caption = element_text(size = 14, face = "italic", family = "Times"))

ggsave("country-interaction-plots.png", plots, width=17, height=10, units= "in" ,dpi=300)
```
