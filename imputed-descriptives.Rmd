...
cohort_nvq =  as_tibble(cohort_nvq) %>% 
  rename("Proportion (%) or \n Mean(±SD) [95% CIs]" = value) %>% 
           add_column(Variable = c("Parent Education \n (NVQ1)",
                                   "Parent Education \n (None of these/overseas qualifications)",
                                   "Parent Education \n (NVQ2)",
                                   "Parent Education \n (NVQ3)",
                                   "Parent Education \n (NVQ4)", 
                                   "Parent Education \n (NVQ5)"), .before = 1)
...
mcs_cohort$benchmark_binary = relevel(mcs_cohort$benchmark_binary, ref = "1")
...
cohort_benchmark =  as_tibble(cohort_benchmark) %>% 
  rename("Proportion (%) or \n Mean(±SD) [95% CIs]" = value) %>% 
           add_column(Variable = c("Core Subjects Grade ≥ 4: Yes",
                                   "Core Subjects Grade ≥ 4: No"), .before = 1)
...
cohort_ed = round(get_weighted_descriptives(mcs_cohort[!is.na(mcs_cohort$standardised_core_subjects),]$standardised_core_subjects,
                          mcs_cohort[!is.na(mcs_cohort$standardised_core_subjects),]$weight),2)

cohort_ed = as.data.frame(t(cohort_ed)) %>% 
  rename("mean" = V1, 
         "sd" = V2, 
         "lower" = V3, 
         "upper" = V4) 
cohort_ed = cohort_ed %>% add_column(paste0(cohort_ed$mean,"(±",cohort_ed$sd,")", 
                                                  "\n", "[", cohort_ed$lower, ";", cohort_ed$upper,"]")) %>% 
  rename("Proportion (%) or \n Mean(±SD) [95% CIs]" = `paste0(...)`) %>% 
  select(`Proportion (%) or \n Mean(±SD) [95% CIs]`) %>% 
  add_column(Variable = c("Continuous Outcome"), .before = 1)

descriptivesCohort = rbind(cohort_vocab, cohort_caregiverVocab, cohort_benchmark, cohort_ed, 
  ...
  add_row(Variable = "Demographics", .before = 8) %>% 
  add_row(Variable = "Socioeconomic Circumstances", .before = 24) %>% 
  add_row(Variable = "Wealth Quntile 1", .after = 35) %>% 
  add_row(Variable = "Wealth Quntile 2", .after = 36) %>% 
  add_row(Variable = "Wealth Quntile 3", .after = 37) %>% 
  add_row(Variable = "Wealth Quntile 4", .after = 38) %>% 
  add_row(Variable = "Wealth Quntile 5", .after = 39) 
