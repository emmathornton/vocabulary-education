...
load("~/Documents/PhD/updated MCS datasets/education-datasets/2021-12-20_vocabulary_education_imputedMAIN/2021-12-20_vocabulary_education_imputedMAIN.Rdata")
imputed_mcs2 = mi.res
...
binary_unadjusted = with(imputed_mcs2, glm(success ~ age5_standardised,
...
compositeQuintiles_plot = plot(pool_predictions(composite_quintiles), ci = FALSE,
  ...
  labs(x = "Age 5 Vocabulary Score \n (Standardised)", 
       ...
       caption = "*Predicted probabilities when sex, ethnicity, EAL status and country set to reference levels and mean caregiver vocabulary score.\n See table SX for predicted values") +
...
ggsave("composite-moderator.png", compositeQuintiles_plot, width=10, height=9, units= "in" ,dpi=300)

#plot for combined plot with other indicators 
...
plots = plots + labs(caption ="*Predicted probabilities when categorical potential confounders (sex, ethnicity, EAL status, country and remaining SEC indicators for individual indicator moderatiins) set to reference levels and mean caregiver vocabulary score.\n See table SX for predicted probabilities values") +
  theme(plot.caption = element_text(size = 10, face = "italic", family = "Times"))

ggsave("interaction-plots.png", plots, width=12, height=10, units= "in" ,dpi=300)
```
...
nvq_predictions = nvq_predictions %>% group_by(x) %>% 
  select(x, group, value) %>% 
pivot_wider(names_from = x, values_from = c("value")) %>% 
  select(group, `-2.96`, `-1.99`, `-0.94`, `0.12`, `2.31`) %>% 
  rename("SEC Indicator" = group) %>% 
...
NVQ_moderatorSensitivity = with(imputed_mcs2, glm(success ~ sex + ethnicity + EAL + country +
                                                          oecd_income*age5_standardised + oecd_income*highest_nvq + 
                                                    occupational_status*age5_standardised +occupational_status*highest_nvq +
                                                    wealth_quintiles*age5_standardised + wealth_quintiles*highest_nvq + 
                                                    imd*age5_standardised + imd*highest_nvq + 
                                         caregiver_vocabStandardised + highest_nvq*age5_standardised, 
                                         family = binomial, weights = weight))
...
occupation_moderatorSensitivity = with(imputed_mcs2, glm(success ~ sex + ethnicity + EAL + country +
                                                           highest_nvq*age5_standardised + highest_nvq*occupational_status+
                                                           oecd_income*age5_standardised + oecd_income*occupational_status + 
                                                    wealth_quintiles*age5_standardised + wealth_quintiles*occupational_status + 
                                                    imd*age5_standardised + imd*occupational_status + 
                                         caregiver_vocabStandardised + occupational_status*age5_standardised, 
                                       family = binomial, weights = weight))
...
income_moderatorSensitivity = with(imputed_mcs2, glm(success ~ sex + ethnicity + EAL + country +
                                                       highest_nvq*age5_standardised + highest_nvq*oecd_income+
                                                    occupational_status*age5_standardised +occupational_status*oecd_income +
                                                    wealth_quintiles*age5_standardised + wealth_quintiles*oecd_income + 
                                                    imd*age5_standardised + imd*oecd_income + 
                                                      caregiver_vocabStandardised + oecd_income*age5_standardised, 
                                                    family = binomial, weights = weight))
...
wealth_moderatorSensitivity = with(imputed_mcs2, glm(success ~ sex + ethnicity + EAL + country +
                                            highest_nvq*age5_standardised + highest_nvq*wealth_quintiles +
                                              oecd_income*age5_standardised + oecd_income*wealth_quintiles + 
                                              occupational_status*age5_standardised + occupational_status*wealth_quintiles +
                                              imd*age5_standardised + imd*wealth_quintiles + 
                                            caregiver_vocabStandardised + wealth_quintiles*age5_standardised, 
                                          family = binomial, weights = weight))
...
imd_moderatorSensitivity  = with(imputed_mcs2, glm(success ~ sex + ethnicity + EAL + country +
                                            highest_nvq*age5_standardised + highest_nvq*imd +
                                              oecd_income*age5_standardised + oecd_income*imd + 
                                              occupational_status*age5_standardised +occupational_status*imd +
                                              wealth_quintiles*age5_standardised + wealth_quintiles*imd + 
                                            caregiver_vocabStandardised + imd*age5_standardised, 
                                          family = binomial, weights = weight))
